...
namespace Client {
    public class Reader {
        ...
        static private int StartLeft = 0;
        ...
        static int RenderText(string text) {
            int left = Console.CursorLeft - InputIndex;
            if (left < 0)
            ...
            int clearCount = Console.BufferWidth - Console.CursorLeft - 1;
            Console.Write(String.Concat(Enumerable.Repeat(" ", clearCount)));
            left = Console.CursorLeft - (Input.Length - InputIndex + clearCount);
            int trueLeft = left;
            if (left < 0)
                left = 0;
            Console.CursorLeft = left;
